"""
capital issue fixed, open trade and capital deployed was wrong
...
def main():
    ...
    max_hold   = cfg["backtest"].get("max_hold_days", 400)
    cap        = cfg["capital"]["initial_capital"]
    max_pos    = cfg["capital"].get("max_position_pct", 30.0)
    ...
    # Process ALL stocks day by day together with shared portfolio state.
    # Enforces:
    #   max_open_trades : never exceed N simultaneous positions
    #   capital limit   : never deploy more than initial_capital in total
    #   max_position_pct: never put more than X% in one stock
    # Previously each stock ran independently → 22 simultaneous trades on ₹5L
    # Now: shared portfolio state → realistic capital constraint

    print("\n  Running portfolio-level backtest...\n")
    print(f"  Capital: ₹{cap:,} | Max trades: {max_trades} | Max per trade: {max_pos:.0f}% = ₹{max_pos/100*cap:,.0f}\n")

    # Portfolio state — shared across all stocks
    open_positions = {}   # {symbol: {entry_date, entry_price, sl, target, qty, ...}}
    all_trades     = []
    skipped_signals= 0

    # Pre-index all stock data by date for fast lookup
    date_index = {}  # {date: {symbol: row_index}}
    for sym, df in all_data.items():
        for idx, row in df.iterrows():
            d = row["date"]
            if d not in date_index:
                date_index[d] = {}
            date_index[d][sym] = idx

    rr           = cfg["trade_rules"]["reward_to_risk_ratio"]
    max_hold     = cfg["backtest"].get("max_hold_days", 400)
    close_last   = cfg["backtest"]["close_all_on_last_day"]
    ltcg_protect = cfg["exit_rules"].get("ltcg_protect", True)
    ltcg_days    = cfg["backtest"].get("ltcg_hold_days", 366)
    do_partial   = cfg["exit_rules"].get("partial_exit", False)
    move_be      = cfg["exit_rules"].get("move_sl_to_breakeven", True)
    do_trail     = cfg["exit_rules"].get("trailing_stop", True)
    trail_atr    = cfg["exit_rules"].get("trailing_stop_atr_multiplier", 2.0)
    caution_score= cfg.get("volatility_filter", {}).get("vix_caution_min_score", 0.65)
    max_pos_val  = cap * (cfg["capital"].get("max_position_pct", 30.0) / 100)
    risk_amt     = cap * cfg["capital"]["risk_per_trade_pct"] / 100
    ...
    for dt in all_dates:
        ...
        # ── 1. Process exits for all open positions first ──
        to_remove = []
        for sym, pos in open_positions.items():
            if sym not in date_index.get(dt, {}):
                continue
            idx      = date_index[dt][sym]
            df       = all_data[sym]
            if idx >= len(df) - 1:
                continue
            row      = df.iloc[idx]
            next_row = df.iloc[idx + 1]
            is_last  = (idx == len(df) - 2)

            hold_days   = (pd.to_datetime(next_row["date"]) -
                           pd.to_datetime(pos["entry_date"])).days
            current_pnl = (next_row["open"] - pos["entry_price"]) * pos["qty"]
            sl          = pos["sl"]
            target      = pos["target"]
            partial_done= pos.get("partial_done", False)

            # Trailing stop (only after partial_done)
            if do_trail and partial_done:
                trail_sl = next_row["close"] - row["atr"] * trail_atr
                if trail_sl > sl:
                    sl = round(trail_sl, 2)
            ...
            if (is_last and close_last) or hold_days >= max_hold:
                if ltcg_protect and hold_days < ltcg_days and current_pnl > 0 and not is_last:
                    continue  # extend hold for LTCG
                reason = "forced_close" if (is_last and close_last) else "max_hold_exit"
                all_trades.append(_build_trade(
                    sym, pos["entry_date"], next_row["date"],
                    pos["entry_price"], next_row["open"],
                    pos["qty"], sl, target, reason, cfg))
        ...
        deployed = sum(p["buy_value"] for p in open_positions.values())
        available = cap - deployed
        if available < max_pos_val * 0.5:
            continue  # less than half a position available → skip

        # Get today's ranked eligible stocks (already ranked in eligible dict)
        # Sort by rank_score (get from daily_rankings)
        today_rankings = [r for r in daily_rankings if r["date"] == dt]
        today_rankings.sort(key=lambda x: x["rank_score"], reverse=True)

        for ranked_row in today_rankings:
            sym = ranked_row["symbol"]
            ...
            # Skip if not in eligible set
            if dt not in eligible.get(sym, set()):
                continue
            ...
            if not row.get("in_trading_period", True):
                continue
            if not row.get("signal", False):
                continue

            # VIX filter
            vix_info = vix_map.get(dt)
            if vix_info:
                vix_avg, regime = vix_info
                if regime == "no_entry":
                    continue
                if regime == "caution" and ranked_row["rank_score"] < caution_score:
                    continue

            # Nifty trend filter
            if nifty_trend:
                info = nifty_trend.get(dt)
                if info and not info.get("trend_ok", True):
                    continue

            # Limit order entry — fill only if next day low <= signal day close
            limit_price = row["close"]
            if next_row["low"] > limit_price:
                skipped_signals += 1
                continue  # gap up — limit not filled

            entry_price = limit_price
            sl_price    = get_sl(row, entry_price, cfg)
            if entry_price <= sl_price or (entry_price - sl_price) < 0.5:
                continue

            risk        = entry_price - sl_price
            target_p    = round(entry_price + risk * rr, 2)

            # Position sizing
            qty_by_risk = int(risk_amt / risk) if risk > 0 else 0
            qty_by_cap  = int(max_pos_val / entry_price) if entry_price > 0 else 0
            qty         = max(1, min(qty_by_risk, qty_by_cap))
            buy_value   = round(qty * entry_price, 2)

            # ── CAPITAL CHECK (the real fix) ──
            if deployed + buy_value > cap:
                skipped_signals += 1
            ...
            print(f"  BUY  {next_row['date']} | {sym:15s} | "
                  f"₹{entry_price:>8,.2f} × {qty:3d} = ₹{buy_value:>8,.0f} | "
                  f"Open:{len(open_positions)}/{max_trades} | "
                  f"Deployed:₹{deployed:,.0f}")

    # ── Close any remaining open positions at end of data ──
    last_date = all_dates[-1]
    for sym, pos in list(open_positions.items()):
        if sym not in date_index.get(last_date, {}):
            continue
        idx = date_index[last_date][sym]
        df  = all_data[sym]
        if idx >= len(df) - 1:
            continue
        next_row = df.iloc[idx + 1]
        ...
        all_trades.append(_build_trade(
            sym, pos["entry_date"], next_row["date"],
            pos["entry_price"], next_row["open"],
            pos["qty"], pos["sl"], pos["target"], "forced_close", cfg))

    print(f"\n  Signals skipped (capital/slot full) : {skipped_signals}")
