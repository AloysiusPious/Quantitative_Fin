...
def load_instruments(enctoken: str) -> pd.DataFrame:
    """
    Load NSE instruments with 3-level fallback:
      1. Live download from kite.zerodha.com/oms/instruments/NSE (enctoken auth)
      2. Local instruments.csv in same directory (if live fails)
      3. Hardcoded token map for all Nifty 50 stocks (last resort)

    Saves downloaded file as instruments.csv for future fallback.
    """
    csv_path = Path("instruments.csv")

    # ── Level 1: Live download ──
    print("  ↓ NSE instruments ...", end=" ", flush=True)
    try:
        r = SESSION.get(
            "https://kite.zerodha.com/oms/instruments/NSE",
            headers=get_headers(enctoken),
            timeout=20
        )
        df = pd.read_csv(pd.io.common.StringIO(r.text))
        if "tradingsymbol" in df.columns and len(df) > 100:
            df.to_csv(csv_path, index=False)   # save for future fallback
            print(f"✓  live ({len(df):,} instruments, saved to instruments.csv)")
            return df
        else:
            raise ValueError(f"Unexpected response: {r.text[:80]}")
    except Exception as e:
        print(f"✗  ({e})")

    # ── Level 2: Local instruments.csv ──
    if csv_path.exists():
        try:
            df = pd.read_csv(csv_path)
            if "tradingsymbol" in df.columns and len(df) > 100:
                age_hours = (datetime.now().timestamp() - csv_path.stat().st_mtime) / 3600
                print(f"  ↓ NSE instruments ... ✓  local instruments.csv ({len(df):,} rows, {age_hours:.0f}h old)")
                return df
        except Exception as e:
            print(f"  ⚠ instruments.csv load failed: {e}")

    # ── Level 3: Hardcoded Nifty 50 token map ──
    print("  ↓ NSE instruments ... using hardcoded token map (fallback)")
    tokens = {
        "ADANIPORTS":  3861249,  "APOLLOHOSP":  4343041,  "ASIANPAINT":  60417,
        "AXISBANK":    1510401,  "BAJAJ-AUTO":  4267265,  "BAJFINANCE":  81153,
        "BAJAJFINSV":  4268801,  "BPCL":        134657,   "BHARTIARTL":  2714625,
        ...
        "SBIN":        779521,   "SBILIFE":     5582849,  "SHREECEM":    3399937,
        "SUNPHARMA":   857857,   "TATAMOTORS":  884737,   "TATASTEEL":   895745,
        "TATACONSUM":  878593,   "TCS":         2953217,  "TECHM":       3465729,
        "TITAN":       897537,   "ULTRACEMCO":  2952193,  "UPL":         2889473,
        "WIPRO":       969473,   "PIDILITIND":  7096321,
        "NIFTY50":     256265,   "INDIAVIX":    264969,
    }
    rows = [{"tradingsymbol": k, "instrument_token": v,
             "instrument_type": "EQ", "exchange": "NSE"}
            for k, v in tokens.items()]
    return pd.DataFrame(rows)


def get_token(symbol: str, instruments_df: pd.DataFrame) -> int | None:
    """Lookup instrument token from DataFrame. Returns None if not found."""
    ...
    try:
        ...
        m = instruments_df[
            ...
            (instruments_df.get("instrument_type", pd.Series(["EQ"]*len(instruments_df))) == "EQ")
        ]
        if not m.empty:
            return int(m.iloc[0]["instrument_token"])
        # Fallback: any match
        m2 = instruments_df[instruments_df["tradingsymbol"] == symbol]
        if not m2.empty:
            return int(m2.iloc[0]["instrument_token"])
    except Exception as e:
        print(f"    ⚠ Token error {symbol}: {e}")
    return None
...
def save_signals_to_csv(ranked: list, mkt: dict, cfg: dict, rr: float,
                        capital: float, open_positions: int = 0, max_trades: int = 5):
    """
    ...
    rows = []
    slots = max(0, max_trades - open_positions)
    budget_left = slots * (capital * cfg["capital"].get("max_position_pct", 20.0) / 100)
    ...
    for sig in ranked:
        ...
        can_take    = "YES" if (budget_left >= sig["buy_value"] and slots > 0) else "NO-SKIP"
        if can_take == "YES":
            budget_left -= sig["buy_value"]
            slots       -= 1
        ...
        rows.append({
            ...
            "can_take":            can_take,      # YES = take trade, NO-SKIP = no capital/slots
            "open_slots_at_scan":  max(0, max_trades - open_positions),
...
def main():
    ...
    # Load instruments ONCE — all 50 token lookups use this cache
    instruments_df = load_instruments(enctoken)

    all_data = {}
    for sym in symbols:
        token = get_token(sym, instruments_df)
        if not token:
            print(f"  ⚠ {sym:15s} token not found — skipping")
            continue
    ...
    capital     = cfg["capital"]["initial_capital"]
    max_trades  = cfg["capital"]["max_open_trades"]
    max_pos_pct = cfg["capital"].get("max_position_pct", 20.0)
    max_buy_per = capital * max_pos_pct / 100
    rr          = cfg["trade_rules"]["reward_to_risk_ratio"]

    # Count already-open positions from signals_log.csv
    # Open = order_placed=YES and exit_date is empty
    open_positions = 0
    csv_path = Path("signals_log.csv")
    if csv_path.exists():
        try:
            df_log = pd.read_csv(csv_path, dtype=str).fillna("")
            open_pos_df = df_log[
                (df_log["order_placed"].str.upper() == "YES") &
                (df_log["exit_date"] == "")
            ]
    ...
    slots_available  = max(0, max_trades - open_positions)
    capital_per_slot = max_buy_per
    capital_available= slots_available * capital_per_slot

    print(f"  💰 CAPITAL STATUS")
    print(f"     Total capital      : ₹{capital:,.0f}")
    print(f"     Max trades allowed : {max_trades}")
    print(f"     Currently open     : {open_positions} (from signals_log.csv order_placed=YES)")
    print(f"     Slots available    : {slots_available}")
    print(f"     Capital available  : ₹{capital_available:,.0f}  ({slots_available} × ₹{capital_per_slot:,.0f})")
    print("─"*60)
    ...
    if not ranked:
        ...
        if slots_available == 0:
            print(f"  ⛔ {len(ranked)} signal(s) found but ALL {max_trades} SLOTS FULL")
            print(f"     Wait for an existing position to exit before taking new trades")
            print(f"     Signals logged to CSV for reference only")
        else:
            print(f"  🎯 {len(ranked)} SIGNAL(S) — {slots_available} SLOT(S) AVAILABLE")

        print("─"*60)
        print(f"  {'#':<3} {'Symbol':<12} {'Score':>6} {'Entry~':>8} {'SL':>8} {'Risk/sh':>8} {'Qty':>5} {'BuyVal':>9} {'RSI':>5} {'Action':>10}")
        print(f"  {'─'*90}")

        budget_left = capital_available
        for i, sig in enumerate(ranked, 1):
            can_afford  = budget_left >= sig["buy_value"] and slots_available > 0
            action      = "✅ TAKE" if can_afford else "⏭ SKIP (no capital)"
            ...
            print(f"  {caution_flag}{i:<2} {sig['symbol']:<12} {sig['rank_score']:>6.3f} "
                  f"{sig['entry_est']:>8.2f} {sig['sl']:>8.2f} {sig['risk_per_sh']:>8.2f} "
                  f"{sig['qty']:>5} {sig['buy_value']:>9,.0f} {sig['rsi']:>5.1f} {action:>10}")

        print()
        print("  TRADE DETAILS (TAKE signals only):")
        print("─"*60)

        budget_left     = capital_available
        slots_remaining = max(0, max_trades - open_positions)

        for sig in ranked:
            can_afford = budget_left >= sig["buy_value"] and slots_remaining > 0
            if can_afford:
                budget_left     -= sig["buy_value"]
                slots_remaining -= 1
            risk_total = sig["risk_per_sh"] * sig["qty"]
            status = "✅ TAKE" if can_afford else "⏭ SKIP"
            print(f"\n  {status} 📌 {sig['symbol']}")
            if can_afford:
                print(f"     Entry tomorrow open : ~₹{sig['entry_est']:,.2f}")
                print(f"     Stop Loss           :  ₹{sig['sl']:,.2f}  ({(sig['entry_est']-sig['sl'])/sig['entry_est']*100:.1f}% below)")
                print(f"     Target (RR={rr})     :  ₹{sig['target']:,.2f}  ({(sig['target']-sig['entry_est'])/sig['entry_est']*100:.1f}% above)")
                ...
                print(f"     RSI today           :  {sig['rsi']:.1f}")
                print(f"     Rank score          :  {sig['rank_score']:.3f}")
            else:
                print(f"     Logged to CSV — take when a slot opens up")

        # ── Append ALL signals to CSV (both TAKE and SKIP) ──
        save_signals_to_csv(ranked, mkt, cfg, rr, capital,
                            open_positions, max_trades)

        print()
        print("─"*60)
        print("  ⚡ ACTION CHECKLIST:")
        print("  [ ] Open Zerodha Kite")
        print("  [ ] Place LIMIT orders for TAKE signals only")
        print("  [ ] Set GTT SL orders immediately after fill")
        print(f"  [ ] Update signals_log.csv: order_placed = YES/NO/SKIP")
        if mkt["vix_regime"] == "caution":
            print(f"  ⚠  VIX caution ({mkt['vix']}) — prefer highest ranked signal only")
