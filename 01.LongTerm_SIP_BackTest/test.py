...
def get_rsi(df, timeperiod=14):
    # Calculate RSI (default period is 14 days)
    df['RSI'] = ta.RSI(df['Close'], timeperiod)
    # Calculate RSI with a custom period (e.g., 10 days)
    df['RSI_' + str(timeperiod)] = ta.RSI(df['Close'], timeperiod)
    # Check if RSI is less than 32
    df['RSI_Less_32'] = df['RSI'] < 32
    print(df['RSI_Less_32'].head(20))
...
def macd_cross_long(filtered_df, i):
    ...
    second_last_row = filtered_df.iloc[-2]

    #print('Cross Below Signal Line')
    #return second_last_row['MACD'] > second_last_row['Signal_Line'] and last_row['MACD'] < last_row['Signal_Line']
...
def low_lt_n_low(filtered_df):
    ...
    today_opn_lt_past_7_days_min = filtered_df['Open'].iloc[-1] < filtered_df['Past_7_Days_Min_Open'].iloc[-1]

    # Print the result
    print(today_opn_lt_past_7_days_min)
    return today_opn_lt_past_7_days_min
...
def get_signal(i, price_ema, filtered_df):
    ...
    macd_cross = macd_cross_long(filtered_df, i)
    ...
    condition_6 = rsi_cross_n(filtered_df, i, 32) and three_black_clo_lt_200ema and three_black_opn_lt_200ema and three_black_clo_below_previous
    condition_7 = rsi_cross_n(filtered_df, i, 29)
    condition_8 = current_clo_lt_200ema and macd_cross_long(filtered_df, i)
    return current_clo_lt_200ema


def main(stock):
    # Retrieve data for the stock from Yahoo Finance
    dataframe = fetch_yahoo_finance_data(stock, from_date,
                                         to_date) if data_source == "yahoo" else get_data_from_zerodha(stock, from_date,
                                                                                                       to_date,
                                                                                                       time_frame)
    ...
    # Print the DataFrame to see the RSI values and the new column
    # print(filtered_df[['RSI_Less_32']]
    ...
    for i in range(EMA_PERIOD, len(filtered_df)):
        TO_DIVIDE = 1 if get_signal(i, price_ema, filtered_df) else 2
        if True:
            ...
            num_stocks_to_buy = int((AMOUNT_TO_INVEST/TO_DIVIDE) // filtered_df['Close'].iloc[i])
    ...
    print('Total Cumulative Percentage Return:', f'{total_cumulative_percentage:.2f}% for Symbol :{stock}')
...
if __name__ == "__main__":
    ...
    # symbols_file = "lt_bookval_3x.txt"
