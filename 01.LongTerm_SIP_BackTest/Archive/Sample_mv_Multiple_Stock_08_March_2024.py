...
sys.path.append('../../../Archive/Archive/my_lib')
from yfin import *
    ...
    for stock in stocks:
        ...
        for i in range(EMA_PERIOD, len(filtered_df)):
            if price_ema.iloc[i] > filtered_df['Close'].iloc[i]:
                ...
                cand_curr_red_prev_green = filtered_df['Close'].iloc[i] < filtered_df['Open'].iloc[i] and filtered_df['Close'].iloc[i - 1] > filtered_df['Open'].iloc[i - 1]
                if cand_curr_red_prev_green: #and volume_spike(filtered_df, i):
                    ...
                    if pd.isna(master_df.at[stock, month_year]):
                        master_df[month_year] = pd.to_datetime(master_df[month_year], errors='coerce')
                        master_df.at[stock, month_year] = pd.to_datetime(filtered_df['Date'].iloc[i]).strftime('%d-%m-%Y')
                        master_df.to_csv(master_csv_path)

                        # Append buy information to lists
                        master_df.at[stock, month_year] = filtered_df['Date'].iloc[i].strftime('%d-%m-%Y')
