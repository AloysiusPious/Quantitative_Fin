...
################
sys.path.append('../../Archive/zerodha_lib')
...
def analyze_investments(directory):
    ...
    result_file_path = os.path.join(Master_Dir, 'investment_analysis.csv')
...
def current_clo_gt_than_past_n_days(filtered_df, i, n):
    # Check if current close is greater than the past 5 days' close
    past_n_days_close = filtered_df['Close'].iloc[i - n:i].mean()
    is_close_greater_than_past_n_days = filtered_df['Close'].iloc[i] > past_n_days_close

    # Combine conditions
    rsi_cross_condition = is_close_greater_than_past_n_days

    return rsi_cross_condition
...
def macd_cross_long(df, i):
    ...
    # Check for MACD crossing above Signal Line using iloc
    last_row = df.iloc[i]
    second_last_row = df.iloc[i - 1]

    macd_above_signal = (second_last_row['MACD'] <= second_last_row['Signal_Line']) and (
...
def get_signal(i, filtered_df):
    ...
    condition_1 = current_clo_green and three_prev_clo_lt_200ema and current_clo_gt_200ema
    condition_2 = current_clo_green and three_prev_clo_lt_200ema and rsi_cross_n(filtered_df, i, 32)
    condition_3 = current_clo_green and three_prev_clo_lt_200ema and current_clo_gt_than_past_n_days(filtered_df, i, 5)
    condition_4 = three_black_clo_lt_200ema and three_black_opn_lt_200ema and three_black_clo_below_previous
    condition_5 = vol_spike and current_clo_green and three_hig_lt_200ema
    condition_6 = current_clo_lt_200ema and (rsi_cross_n(filtered_df, i, 29) or rsi_cross_n(filtered_df, i, 33))
    condition_7 = current_clo_green and current_clo_lt_200ema and macd_cross_long(filtered_df, i)
    ########## RSI and MACD #########
    condition_8 = rsi_below_n(filtered_df, i, 40) and macd_cross_long(filtered_df, i)
    condition_9 = current_clo_green and prev_1_red and current_clo_lt_200ema and macd_cross_long(filtered_df, i)
    return condition_6
...
def start_calc():
    ...
    for stock in stocks:
        # Retrieve data for the stock from Yahoo Finance or other source
        ...
        buy_qty = []  # List to store buy quantities

        # Close details initialized with to_date or the last available date
        if to_date in filtered_df['Date'].values:
            close_date = to_date
            close_price = round(filtered_df.loc[filtered_df['Date'] == to_date, 'Close'].values[0], 2)
        else:
            close_date = filtered_df['Date'].iloc[-1]
            close_price = round(filtered_df['Close'].iloc[-1], 2)

        close_dates = [close_date] * len(filtered_df)  # List to store close dates
        close_prices = [close_price] * len(filtered_df)  # List to store close prices
        ...
        for i in range(EMA_PERIOD, len(filtered_df)):
            if get_signal(i, filtered_df):
                ...
                # Calculate profit
                profit = (close_price - filtered_df['Close'].iloc[i]) * num_stocks_to_buy
        ...
        # Create a DataFrame for buy dates, prices, quantities, and sell details
        buy_df = pd.DataFrame({
            'Buy Date': buy_dates,
            'Buy Price': buy_prices,
            'Buy Qty': buy_qty,
            'Closed Date': close_dates[:len(buy_dates)],  # Ensure same length
            'Close Price': close_prices[:len(buy_dates)],  # Ensure same length
            'Profit': profits[:len(buy_dates)]  # Ensure same length
        })
